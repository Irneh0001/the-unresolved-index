...
Events are noise (repetition), context (background), material (meaningful development), structural (law/ruling/election/treaty or comparable shift), or a resolution candidate. A candidate requires human review against the topic’s definition of resolved.

Assessments retain five 0–100 dimensions: importance, substantive uncertainty, institutional disagreement, decision incompleteness, and momentum. Any derived priority is a transparent heuristic, not an objective unresolved score. The research queue uses `importance × uncertainty × momentum / 10,000`.

Published forecasts state a horizon, defined base and alternative cases, probabilities, confidence, rationale, and evidence. Confidence concerns the assessment quality, not outcome likelihood. Forecast records are append-only; changed evidence creates a new record and preserves the old one for calibration.

Corrections record the affected record, original value, corrected value, reason, source, and timestamp. Historical assessments, forecasts, ledger entries, and corrections are never silently rewritten.
